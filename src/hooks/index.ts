...
export async function fetchTasks(): Promise<Task[]> {
	try {
		const response = await fetch(
			"https://todo-list-back-ftbmfnbrgxc0g2em.eastasia-01.azurewebsites.net/api/tasks",
		);
		if (!response.ok) {
...
export function useTaskManager(initialTasks: Task[]) {
	...
	const addTasks = useCallback(
		async (newTasks: Omit<Task, "id">[]) => {
			...
			try {
				const response = await fetch(
					"https://todo-list-back-ftbmfnbrgxc0g2em.eastasia-01.azurewebsites.net/api/tasks/bulk-create",
					{
						method: "POST",
	...
	const updateTask = useCallback(
		async (updatedTask: Task) => {
			...
			try {
				const response = await fetch(
					`https://todo-list-back-ftbmfnbrgxc0g2em.eastasia-01.azurewebsites.net/api/tasks/${updatedTask.id}`,
					{
						method: "PUT",
						headers: { "Content-Type": "application/json" },
	...
	const toggleTaskComplete = useCallback(
		async (taskId: number) => {
			...
			try {
				const response = await fetch(
					`https://todo-list-back-ftbmfnbrgxc0g2em.eastasia-01.azurewebsites.net/api/tasks/${taskId}/toggle-complete`,
					{
						method: "PATCH",
						headers: { "Content-Type": "application/json" },
						body: JSON.stringify({
	...
	const deleteTask = useCallback(
		async (taskId: number) => {
			...
			try {
				const response = await fetch(
					`https://todo-list-back-ftbmfnbrgxc0g2em.eastasia-01.azurewebsites.net/api/tasks/${taskId}`,
					{
						method: "DELETE",
					},
				);
